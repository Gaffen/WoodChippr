...
function writeJSON(args){
        ...
        "package": {
          ...
          "dist": {
            ...
            "url":
              ...
              args.acfVersion + "&p=pro&a=download&k=" + args.acfKey
          }
        }
      }
    ],
  ...
  fs.writeFile("./composer-merge/acf.json",
    JSON.stringify(acfRepo, null, 2), function(err){
    if(err){
      console.log(err);
      return exit();
    }

    console.log(colors.yellow("Your ACF configuration is complete, just run ") + colors.red.underline("composer install") + colors.yellow(" to finish setting up WoodChippr!"));
...
console.log(colors.white('Welcome to the WoodChippr init script!\n\n' +
...
prompt.get({
  properties:{
    acf: {
      description: colors.yellow("Do you want do configure ACF Pro for this install? (y/N)"),
      default: "n",
      type: "string"
    },
    acfKey: {
    ...
    acfVersion: {
      ...
      ask: function(){
        return (prompt.history('acfKey') !== null &&
          prompt.history('acfKey').value.length > 0)? true : false;
      }
    }
  }
}, function(err, result){
  if(!err){
    if(result.acf === "n"){
      return exit();
    } else if (result.acf.toLowerCase()[0] === "y" &&
      result.acfKey !== ""){
